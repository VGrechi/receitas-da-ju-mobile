...
export const RecipePage: React.FC<RecipePageProps> = ({ route }) => {
  ...
  const [name, setName] = useState(
    route.params.recipe ? route.params.recipe.name : ""
  );
  const [nameError, setNameError] = useState<string>();
  const [difficulty, setDifficulty] = useState(
    route.params.recipe
      ? route.params.recipe.difficulty
      : difficultyLabels[0].label
  );
  const [preparationTime, setPreparationTime] = useState(
    route.params.recipe
      ? route.params.recipe.preparationTime
      : timeLabels[0].label
  );

  const [allIngredients, setAllIngredients] = useState<SingleIngredient[]>([]);
  const [ingredients, setIngredients] = useState(
    route.params.recipe ? route.params.recipe.ingredients : []
  );
  ...
  const [ingredientPercentageError, setIngredientPercentageError] =
    useState<string>();
  const [ingredientValue, setIngredientValue] = useState("");
  const [ingredientValueError, setIngredientValueError] = useState<string>();

  const [preparation, setPreparation] = useState(
    route.params.recipe ? route.params.recipe.preparation : ""
  );
  ...
  return (
    <View style={styles.container}>
      <PageHeader
        title={recipe ? recipe.name : "Nova Receita"}
      ...
      >
        <View style={styles.generalContainer}>
          ...
          />
          {nameError && <Text style={styles.inputLabelError}>{nameError}</Text>}
        ...
        <View style={styles.preparationContainer}>
          <TextInput
            style={[styles.input, styles.inputPreparation]}
        ...
        >
          ...
          <View style={[styles.modalValuesContainer, {}]}>
            <TextInput
              style={[
                styles.input,
                ingredientPercentageError ? styles.inputError : {},
                { flex: 5 },
              ]}
            ...
            <TextInput
              style={[
                styles.input,
                ingredientValueError ? styles.inputError : {},
                { flex: 5 },
              ]}
          ...
          <View>
            {ingredientPercentageError && (
              <Text style={styles.inputLabelError}>
                {ingredientPercentageError}
              </Text>
            )}
